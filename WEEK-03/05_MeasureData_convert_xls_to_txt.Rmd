...
path.my.measure = "C:/Users/michaela.bayerlova/Documents/STATS 419/measurement_project";   # trailing slash or not?

file.my.measure = paste0(path.my.measure,"/","mymeasure.xlsx"); # if no trailing slash, notice the "/"


covariates = read_excel(file.my.measure, sheet="covariates");
panel = read_excel(file.my.measure, sheet="panel");
...
for(i in 1:n.cov)
  ...
  final.row$`person id` = person = covariates.row$`person id`;
  final.row[27:37] = covariates.row[3:13]; 

collector = as.data.frame(panel$`data collector`);
person = as.data.frame(panel$`person id`);
   subpanel = panel[collector & person, ];
   
  #subpanel = panel[panel$`data collector` == collector & panel$`person id` == person, ];
...
collector = "7348fcf09fee6bfa71cd8fafb517cc56";
outfile = paste0(path.my.measure,"/","measure","-",collector,".txt");
